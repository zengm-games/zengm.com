...
<li><b>Playoffs.</b> Teams that made the playoffs ${bySport({
    basketball: '(top 8 in each conference) progress through the bracket playing best-of-7 series until a champion emerges',
...
${bySport({
    basketball: '<li><b>Draft lottery.</b> After the playoffs end, the draft order is set with a lottery.</li>',
    football: '',
    hockey: '<li><b>Draft lottery.</b> After the playoffs end, the draft order is set with a lottery.</li>',
}, sport)}
...
<p>The maximum contract amount is $${bySport({
    ...
    football: '30',
    hockey: '13',
}, sport)} million per year and the maximum contract length is 5 years.</p>
<p>The minimum contract amount is $${bySport({
    basketball: '750',
    football: '500',
    hockey: '500',
}, sport)} thousand per year and the minimum contract length is 1 year (or, until the end of the season, if the season is already in progress).</p>
<p>When a contract expires, you have the opportunity to negotiate a new contract with the player. If you don't come to an agreement, the player becomes a free agent.${sport === "basketball" ? " This is important because, based on the salary cap rules, you can go over the cap to re-sign your own players but you can't go over the cap to sign a free agent." : ""}</p>
...
<p>However, the overall ratings aren't a guarantee of performance. The particular mix of ratings plays into success, as do a player's teammates${sport === "basketball" ? " (<i>e.g.</i> a good rebounder doesn't help your team as much if you already have a few other good rebounders)" : ""}.</p>
...
<p>Finally, little symbols you see next to a player's name like <span class="skills-alone">${Object.entries(skills).map(([skill, description]) => `<span title="${description}" class="skill">${skill}</span>`).join("")}</span> represent the key skills a player has. This is designed so that you can just glance at a player and easily take in that information. It has no impact on gameplay. A player who barely misses getting a skill symbol will be about the same as a player who barely gets it.</p>
<p>To see what a symbol means, hover your mouse over it or consult this list:</p>
<ul>
${Object.entries(skills).map(([skill, description]) => `<li>${skill}: ${description}</li>`).join("")}
</ul>
${bySport({
    basketball: "",
